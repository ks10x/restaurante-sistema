...
$serializePedido = function ($pedido) {
    return [
        ...
        'cliente' => $pedido->usuario->name ?? $pedido->nome_cliente_avulso ?? 'Cliente',
...
function renderPedido(p){return `<article class="card ${statusCardClass(p.status)} ${p.faixa_tempo==='critico'?'critico':''}" data-id="${p.id}"><div class="progress"><span style="width:${p.progresso}%;background:${statusProgressColor(p.status)}"></span></div><div class="card-main"><div class="card-top"><div><div class="code">#${escapeHtml(p.codigo)}</div><div class="client">${escapeHtml(p.cliente||'Cliente')} · ${p.tipo_consumo==='local'?`<b style="color:#d97706">MESA ${escapeHtml(p.mesa_numero||'')}</b>`:(p.tipo_entrega==='entrega'?'Entrega':'Retirada')}</div></div><div class="timer ${timerClass(p.faixa_tempo)}">${Math.round(Number(p.minutos_decorridos||0))} min</div></div><div class="items">${p.itens.map(item=>`<div class="item"><img src="${escapeHtml(item.imagem||'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=500&auto=format&fit=crop')}" alt="${escapeHtml(item.nome||'Prato')}" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=500&auto=format&fit=crop';"><div><div class="item-name">${item.quantidade}x ${escapeHtml(item.nome)}</div>${item.descricao?`<div class="item-desc">${escapeHtml(item.descricao)}</div>`:''}<div class="tags">${(item.ingredientes||[]).slice(0,4).map(i=>`<span class="tag">${escapeHtml(i)}</span>`).join('')}${(item.opcoes||[]).slice(0,2).map(op=>`<span class="tag">${escapeHtml(op.nome||'Opcao')}: ${escapeHtml(op.valor||'')}</span>`).join('')}</div>${item.observacao?`<div class="item-meta">Observacao: ${escapeHtml(item.observacao)}</div>`:''}</div></div>`).join('')}</div>${p.tipo_consumo==='local'?`<div class="note address" style="background:#fef3c7;border:1px solid #fde68a;">Pedido Local (Mesas)</div>`:(p.endereco?.texto?`<div class="note address">${escapeHtml(p.endereco.texto)}</div>`:`<div class="note address">Retirada no balcao</div>`)}${p.observacoes?`<div class="note obs">${escapeHtml(p.observacoes)}</div>`:''}<div class="actions">${p.status!=='saindo_entrega'?`<button class="btn btn-primary" data-action="advance" data-url="${escapeHtml(p.avancar_url)}">${p.status==='confirmado'?'Iniciar producao':'Pronto para Entregar'}</button><button class="btn btn-danger" data-action="cancel" data-url="${escapeHtml(p.cancelar_url)}">Cancelar</button>`:`<button class="btn btn-primary" data-action="advance" data-url="${escapeHtml(p.avancar_url)}">Finalizar / Marcar Entregue</button>`}</div></div></article>`;}
