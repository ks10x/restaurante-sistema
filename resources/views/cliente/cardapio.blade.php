    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .cart-count {
            ...
            border: 2px solid var(--dark); font-weight: 800;
        }
        ...
        /* MAIN LAYOUT */
        .main-wrap{max-width:1200px;margin:0 auto;padding:2.5rem 2rem 4rem;display:grid;grid-template-columns:220px 1fr;gap:2.5rem;}
        ...
        .cat-item{
            ...
            font-size:.9rem;color:var(--text-m);font-weight:400; border:1px solid transparent;text-decoration:none;
        }
        .cat-item:hover, .cat-item.active{background:var(--surface);border-color:var(--border);color:var(--amber)}
        ...
        .btn-add{background:var(--amber);color:var(--dark);border:none;border-radius:10px;width:40px;height:40px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:1.4rem; font-weight: bold; transition: 0.2s;}
        .btn-add:hover{background:var(--amber-d); transform: scale(1.05);}

        /* BARRA FLUTUANTE */
        .floating-cart {
            position: fixed; bottom: -80px; left: 50%; transform: translateX(-50%);
            width: 90%; max-width: 500px; background: var(--amber); height: 60px;
            border-radius: 15px; z-index: 250; box-shadow: 0 8px 30px rgba(0,0,0,0.6);
            cursor: pointer; transition: bottom 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);
        }
        .floating-cart.active { bottom: 25px; }
        .floating-cart-inner { display: flex; align-items: center; justify-content: space-between; height: 100%; padding: 0 1.5rem; color: var(--dark); font-weight: 700;}
        ...
        @media(max-width:768px){ .main-wrap{grid-template-columns:1fr;} .sidebar{display:none;} .cart-drawer{width: 100%;} }
    </style>
</head>
<body>
...
<header>
    ...
    <div class="header-actions">
        <button class="btn-sacola" onclick="toggleCart()">
...
<div class="main-wrap">
    <aside class="sidebar">
        ...
        <a href="javascript:void(0)" class="cat-item active" onclick="filtrar('todas')">🍽️ Todas</a>
        @foreach($categorias as $cat)
            <a href="javascript:void(0)" class="cat-item" onclick="filtrar('{{ $cat->id }}')">
                {{ $cat->icone ?? '🍴' }} {{ $cat->nome }}
            </a>
        @endforeach
    </aside>
            ...
            <div class="categoria-section" id="secao-{{ $cat->id }}">
                ...
                <div class="destaques-grid">
                    @foreach($cat->pratos as $p)
                        <div class="card" onclick='abrirModal(@json($p))'>
                            <div class="card-img">
                                <img src="{{ asset('storage/' . $p->imagem) }}" alt="{{ $p->nome }}">
                            </div>
                            <div class="card-body">
                                <div class="card-nome">{{ $p->nome }}</div>
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-top: auto;">
                                    <div class="card-preco">R$ {{ number_format($p->preco, 2, ',', '.') }}</div>
                                    <button class="btn-add" onclick="event.stopPropagation(); addToCart(@json($p))">+</button>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
<div id="floatingCart" class="floating-cart" onclick="toggleCart()">
    <div class="floating-cart-inner">
        <div style="display:flex; align-items:center; gap:12px;">
            <span id="floatQty" style="background:rgba(0,0,0,0.2); padding:4px 12px; border-radius:8px;">0</span>
        ...
        <div style="padding:25px;">
            ...
            <div style="margin-top:30px; display:flex; justify-content:space-between; align-items:center; gap: 20px;">
                <div style="display:flex; gap:15px; align-items:center; background:var(--dark); padding:10px 20px; border-radius:15px; border: 1px solid var(--border);">
                    <button onclick="mudarQtd(-1)" style="background:none; border:none; color:var(--amber); font-size:24px; cursor:pointer;">-</button>
                    <span id="m-qtd" style="font-weight:bold; min-width: 20px; text-align: center;">1</span>
                    <button onclick="mudarQtd(1)" style="background:none; border:none; color:var(--amber); font-size:24px; cursor:pointer;">+</button>
                </div>
                <button onclick="confirmarAdd()" style="background:var(--amber); border:none; padding:18px; border-radius:15px; font-weight:bold; flex:1; cursor:pointer; color: var(--dark); font-size: 1rem;">
...
<div class="cart-drawer" id="cartDrawer">
    <div style="padding:25px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items: center;">
        ...
        <button onclick="toggleCart()" style="background:var(--dark2); border:none; color:var(--text-m); cursor:pointer; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;"><i class="fas fa-times"></i></button>
    </div>
    
    <div id="cartItems" style="flex:1; overflow-y:auto; padding:20px;">
        </div>
    
    <div style="padding:25px; border-top:1px solid var(--border); background: rgba(0,0,0,0.1);">
        ...
        <button onclick="window.location.href='/checkout'" style="width:100%; background:var(--amber); border:none; padding:20px; border-radius:16px; font-weight:800; cursor:pointer; color: var(--dark); font-size: 1.1rem; transition: 0.3s;" onmouseover="this.style.background='#E8A045'" onmouseout="this.style.background='#D4A373'">
            Finalizar Pedido
        </button>
    </div>
</div>
...
<script>
    ...
    // 2. RENDERIZAR INTERFACE DO CARRINHO
    function renderCart() {
        ...
        cart.forEach((item, index) => {
            ...
            container.innerHTML += `
                <div style="display:flex; gap:12px; margin-bottom:15px; align-items:center; background:var(--dark2); padding:12px; border-radius:16px; border: 1px solid var(--border);">
                    <img src="/storage/${item.imagem}" style="width:55px; height:55px; border-radius:10px; object-fit:cover;">
                    <div style="flex:1">
                    ...
                    <button onclick="remover(${index})" style="background:none; border:none; color:var(--red); cursor:pointer; padding: 5px;"><i class="fas fa-trash-alt"></i></button>
                </div>
            `;
        });
        ...
        if(totalQtd > 0) floatingCart.classList.add('active');
        else {
