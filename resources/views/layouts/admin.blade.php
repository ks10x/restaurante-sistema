...
<body class="admin-shell h-full font-sans antialiased" x-data="{ sidebarOpen: false }">

    <div x-show="sidebarOpen" class="relative z-50 lg:hidden" role="dialog" aria-modal="true" style="display: none;">
        <div x-show="sidebarOpen" x-transition.opacity class="fixed inset-0 bg-slate-900/20"></div>

        <div class="fixed inset-0 flex">
            <div x-show="sidebarOpen" x-transition.translate class="relative mr-16 flex w-full max-w-xs flex-1">
                <div class="absolute left-full top-0 flex w-16 justify-center pt-5">
                    <button @click="sidebarOpen = false" type="button" class="-m-2.5 p-2.5 text-slate-700">
                ...
                <div class="admin-sidebar flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4">
                    <div class="flex h-16 shrink-0 items-center admin-sidebar-title text-xl font-bold tracking-tight">
    ...
    <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
        <div class="admin-sidebar flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4">
            <div class="flex h-16 shrink-0 items-center justify-between">
                <span class="admin-sidebar-title text-xl font-bold tracking-tight">{{ config('app.name') }} Admin</span>
                @if(isset($pedidosAtivos) && $pedidosAtivos > 0)
                    <span class="admin-sidebar-badge inline-flex items-center rounded-full px-2 py-1 text-xs font-medium">
    ...
    <div class="lg:pl-72 flex min-h-screen flex-col bg-white">
        <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8" style="border-color: var(--border-soft);">
            ...
            <div class="flex flex-1 justify-end gap-x-4 self-stretch lg:gap-x-6">
                <div class="flex items-center gap-x-4 lg:gap-x-6">
                    <a href="{{ route('admin.configuracoes.index') }}" class="-m-2.5 p-2.5 text-slate-400 transition hover:text-[var(--color-secondary)]">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition class="absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-slate-900/5 focus:outline-none" style="display: none;">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="block w-full px-3 py-1 text-left text-sm leading-6 text-slate-900 hover:bg-slate-50">Sair (Logout)</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        <main class="flex-1 py-10 pt-6">
            <div class="px-4 sm:px-6 lg:px-8">
                @if (session('success'))
                <div class="relative mb-6 rounded-md bg-green-50 p-4" x-data="{ show: true }" x-show="show">
                    <div class="flex">
                ...
                @if (session('error'))
                <div class="relative mb-6 rounded-md bg-red-50 p-4" x-data="{ show: true }" x-show="show">
                    <div class="flex">
            ...
            <div class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                <div class="px-6 py-5">
                    <div class="flex items-start gap-4">
                        <div class="flex h-12 w-12 items-center justify-center rounded-full bg-red-100 text-xl font-bold text-red-600">!</div>
                        <div class="flex-1">
                            <h3 id="admin-confirm-title" class="text-lg font-bold text-slate-900">Confirmar acao</h3>
                            <p id="admin-confirm-message" class="mt-2 text-sm text-slate-600">Tem certeza que deseja continuar?</p>
                        </div>
                    </div>
                </div>
                <div class="flex justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4">
                    <button id="admin-confirm-cancel" type="button" class="rounded-lg border border-slate-200 bg-white px-4 py-2 font-bold text-slate-700 shadow-sm transition-colors hover:bg-slate-50">Cancelar</button>
                    <button id="admin-confirm-ok" type="button" class="rounded-lg bg-red-600 px-5 py-2 font-bold text-white shadow-sm transition-colors hover:bg-red-700">Confirmar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        (() => {
            ...
            window.adminConfirm = function ({
                ...
                confirmButton.className = `rounded-lg px-5 py-2 font-bold text-white shadow-sm transition-colors ${confirmClass}`;
